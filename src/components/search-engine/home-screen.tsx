...
const FEATURES = [
  {
    icon: EyeOff,
    titleAr: "بدون قيود على المنصات",
    titleEn: "Unrestricted Platform Results",
    descAr: "النتائج لا تُفلتر — كل ما هو موجود على الويب يظهر، بما في ذلك المحجوب.",
    descEn: "Results are not filtered — everything on the web appears, including blocked content.",
  },
  {
    icon: Lock,
    titleAr: "توجيه متعدد العناوين",
    titleEn: "Multi-Hop Routing",
    descAr: "حارس دخول ← عقدة وسيطة ← عقدة خروج. كل عقدة في دولة مختلفة.",
    descEn: "Entry guard ← Middle node ← Exit node. Each node in a different country.",
  },
  {
    icon: Bot,
    titleAr: "AI مدمج",
    titleEn: "Built-in AI",
    descAr: "موجز فوري فوق النتائج + مساعد دردشة «شبح AI» يلخّص أي صفحة بضغطة.",
    descEn: "Instant summary above results + Shabah AI chat assistant to summarize any page.",
  },
  {
    icon: Flame,
    titleAr: "جدار ناري نشط",
    titleEn: "Active Firewall",
    descAr: "حظر XSS، CSRF، فحص المنافذ، تعدين العملات، بصمة الإصبع — تلقائيًا.",
    descEn: "Blocks XSS, CSRF, port scanning, crypto mining, fingerprinting — automatically.",
  },
];
...
export function HomeScreen() {
  const { connected, relays, visibleIp, firewallActive } =
    usePrivacyStore();
    ...
    <main className="flex-1 w-full scroll-mt-14 sm:scroll-mt-0 pb-16 sm:pb-8">
      ...
      <section className="grid-bg relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 pt-12 pb-8 sm:pt-16 sm:pb-12 text-center relative">
          ...
          {/* Status pill — only show after client init */}
          {initialized && (
      ...
      <section className="max-w-5xl mx-auto px-4 py-10 sm:py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURES.map((f) => {
            ...
            return (
              <div
                key={lang === 'ar' ? f.titleAr : f.titleEn}
              ...
              >
                ...
                <h3 className="text-sm font-semibold mb-1.5">{lang === 'ar' ? f.titleAr : f.titleEn}</h3>
      ...
      <section className="max-w-5xl mx-auto px-4 pt-10 pb-10 grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* AI CTA */}
        <div className="rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 to-transparent p-5 flex flex-col">
          ...
          <button
            onClick={() => {
              openAIContext("", t("home.aiChatContext"));
              toggleAI(true);
            }}
